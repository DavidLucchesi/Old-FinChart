...
for (var i = 0; i < obj.continents.length; i++) {
	...
	for (var j=0; j< obj.continents[i].sectors.length;j++){
		...
		detailT.push(obj.continents[i].sectors[j].t);
		if(Math.abs(obj.continents[i].sectors[j].g)>maxAbsolueValueGT)
			maxAbsolueValueGT=Math.abs(obj.continents[i].sectors[j].g);
		if(Math.abs(obj.continents[i].sectors[j].t)>maxAbsolueValueGT)
			maxAbsolueValueGT=Math.abs(obj.continents[i].sectors[j].t);
...
lineValue=Math.ceil(maxAbsolueValueGT);
if (lineValue%2==1)
	lineValue +=1;
...
for (var i = 0; i < obj.continents.length; i++) {
	...
	"%</td><td>"+detailT[sectorValueForTable]+"%</td></tr>";
	...
	for (var j = 0; j < sectorsInContinent[i]-1; j++) {
		...
		"M</td><td>"+detailG[sectorValueForTable]+"%</td><td>"+detailT[sectorValueForTable]+"%</td></tr>";
		sectorValueForTable+=1;
	}
}
}
...
var finAngle=0;
var extra = 80/maxAbsolueValueGT;
for (var i = 0; i < detailP.length; i++) {
	context.beginPath();
	if(detailG[i] > 0)
...
for(i=0; i < sectorsNameList.length; i++)
{	  
	var trackerAngle = initAngle+(Math.abs(detailP[i])*Math.PI/totalPonderation);
	var trackerRadius = radius1+extra*Math.abs(detailT[i]);
	finAngle=initAngle+(detailP[i]*2*Math.PI/totalPonderation);
	var dx = cX + trackerRadius * Math.cos(trackerAngle);
	var dy = cY + trackerRadius * Math.sin(trackerAngle);
	context.beginPath();
	if(detailT[i]>0)	
		context.fillStyle='#10E31E';
	else
...
function createContinent(number){
	...
	var newCaseT = document.createElement('td');
	var inputT = document.createElement('input');
	inputT.setAttribute('type', 'number');
	inputT.setAttribute('name', 'inputT' + number);
	inputT.setAttribute('id', 'inputT' + number);
...
document.getElementById("tablebutton").onclick=function(){
	...
	for(i=0; i < linesInTable; i++){
		if (table.childNodes[i].childNodes.length==5)
			...
			for(var j=0;j<continentLines[i+1]-continentLines[i];j++){
				...
				if (j==0){
					var sector={
						...
						t:Number(sectorTRLine.childNodes[4].childNodes[0].value)
					}
				}
				else{
					var sector={
						...
						t:Number(sectorTRLine.childNodes[3].childNodes[0].value)
					}
				}
